    ...
    if (node.type === featurebook.NODE_FILE) {
      ...
      try {
        ...
        fs.writeFileSync(path.join(outputDir, node.path + '.html'), featureTemplateCompiled({
          ...
          metadata: metadata,
          specTree: specTree,
          feature: markdown.descriptionMarkdownToHTML(feature, markdownOptions)
        }));
      } catch (err) {
...
function imageRenderer(href, title, text) {
  return '<img src="' + (isAsset(href) ? removeSchemaPrefix(href) : href) + '" alt="' + text + '">';
  ...
  function removeSchemaPrefix(url) {
    return url.substring(markdown.ASSET_URL_SCHEMA.length);
  }
}

function linkRenderer(href, title, text) {
  return '<a href="' + (isFeature(href) ? removeSchemaPrefixAndAppendHtmlSuffix(href) : href) + '">' + text + '</a>';
  ...
  function removeSchemaPrefixAndAppendHtmlSuffix(url) {
    return url.substring(markdown.FEATURE_URL_SCHEMA.length) + '.html';
  }
}

module.exports = html;
html.$imageRenderer = imageRenderer;
html.$linkRenderer = linkRenderer;
