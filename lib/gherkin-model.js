    ...
    var featureModel = {
        ...
        lexer = new Lexer({
            ...
            doc_string: function onDocString(content_type, string, line) {
                if (lastKeywordWithSteps === SCENARIO || lastKeywordWithSteps === SCENARIO_OUTLINE) {
                    lastScenario().steps.slice(-1)[0].docString = string;
                }
                if (lastKeywordWithSteps === BACKGROUND) {
                    featureModel.background.steps.slice(-1)[0].docString = string;
                }
            },
            row: function onRow(row, line) {
                if (lastKeywordWithRows === EXAMPLES) {
                    lastScenario().examples.slice(-1)[0].dataTable.push(row);
                }
