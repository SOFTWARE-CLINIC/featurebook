...
function execute(params) {
    ...
    app.get('/api/rest/summary/:path?', function (req, res) {
        ...
        summary ? res.send(markdown.toHTML(summary)) : res.status(404).end();
    });
    ...
    app.get('/api/rest/feature/parsed/:path', function (req, res) {
        res.send(featurebook.descriptionMarkdownToHTML(
            featurebook.getFeatureSync(
                path.join(sourceDir, req.params.path)
            )));
    });
