...
function execute(params) {
    ...
    function readSummarySync() {
        ...
        if (fs.existsSync(summaryFilePath)) {
            ...
            summary.contributors = data.contributors;
    ...
    function printShareLink(port) {
        require('dns').lookup(require('os').hostname(), function (err, address) {
            console.log(color.yellow('It is available to all computers in the local network at http://' + address + ':' + port));
        });
    }
}
