...
var MarkdownNode = require('./markdown-node');
...
function build(sourceDir, outputDir) {
  var doc = new PDFDocument(),
    outputFile = path.join(outputDir, 'specification.pdf'),
    metadata = featurebook.readMetadataSync(sourceDir);
  ...
  var featuresTree = featurebook.readSpecTreeSync(sourceDir);
  ...
  function printFolder(node) {
    doc.addPage()
      .fontSize(fontSize.xxLarge)
      .fillColor('black')
      .text(node.name, {underline: true});

    var summary = featurebook.readSummarySync(path.join(sourceDir, node.path));
    if (summary) {
      doc.moveDown(0.5);
  ...
  function printFeature(node) {
    var feature = featurebook.readFeatureSync(path.join(sourceDir, node.path));
    doc.moveDown()
  ...
  function printMarkdown(text) {
    ...
    while (tree.length) {
      new MarkdownNode(tree.shift()).render(doc);
    }
  }
