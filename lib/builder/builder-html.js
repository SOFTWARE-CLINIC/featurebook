...
function build(sourceDir, outputDir) {
  ...
  copyFile(path.join(TEMPLATES_DIR, 'script.js'), path.join(outputDir, 'script.js'));
  copyFile(path.join(TEMPLATES_DIR, 'stylesheet.css'), path.join(outputDir, 'stylesheet.css'));

  fs.writeFileSync(path.join(outputDir, 'spec-tree.html'), specTreeTemplateCompiled(locals));
  fs.writeFileSync(path.join(outputDir, 'index.html'), indexTemplateCompiled(locals));
  ...
  function print(node) {
    if (node.type === 'file') {
      try {
        ...
        fs.writeFileSync(path.join(outputDir, node.path + '.html'), featureTemplateCompiled({feature: feature}));
      } catch (err) {
